...
@available(iOS 17.0, *)
@MainActor
final class MediaListViewTests: XCTestCase {
    ...
    func test_media_list_successfully_loaded() async throws {
        let interactor = MockMediaInteractor()
        interactor.fetchNextPopularPageAsFullListStub = [
            ...
            Movie.make(id: 3,
                       title: "The assassination of Jesse James by the Coward Robert Ford Final Cut Redux Part 2",
                       voteAverage: 5)
        ]
        let viewModel = MediaListViewModel(interactor: interactor, section: .popularMovies)
        await viewModel.fetchFirstPage()
        let sut = MediaListView(viewModel: viewModel,
    ...
    func test_no_internet_error_state_when_fetching_first_page() async throws {
        let interactor = MockMediaInteractor()
        interactor.error = NSError(domain: "No internet connection", code: 999, userInfo: nil)
        let viewModel = MediaListViewModel(interactor: interactor, section: .popularMovies)
        await viewModel.fetchFirstPage()
        let sut = MediaListView(viewModel: viewModel,
                                favoriteMoviesDBStore: dbStore)
